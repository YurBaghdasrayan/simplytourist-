...
use App\Http\Requests\CreateToursTypesRequest;
use App\Http\Requests\UpdateToursTypesRequest;
use App\Models\ToursTypes;
use App\Repositories\ToursTypesRepository;
use App\Http\Controllers\AppBaseController;
use Illuminate\Http\Request;
use Flash;
use Response;

class ToursTypesController extends AppBaseController
{
    /** @var  ToursTypesRepository */
    private $toursTypesRepository;
    private static $slug = 'tours-types';

    public function __construct(ToursTypesRepository $toursTypesRepo)
    {
        $this->toursTypesRepository = $toursTypesRepo;
    }
...
//        $query=ToursTypes::select('*');
//        $dataTypeContent = self::getData(self::$slug, $request, $query);
//        $browserable_rows = self::getFields(self::$slug, $this);
//
//        return view('tours_types.index')
//            ->with(['toursTypes'=>$dataTypeContent,'rows'=>$browserable_rows]);
    }
