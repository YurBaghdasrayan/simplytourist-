...
use App\Http\Requests\CreateTourEquipmentRequest;
use App\Http\Requests\UpdateTourEquipmentRequest;
use App\Models\TourEquipment;
use App\Repositories\TourEquipmentRepository;
...
class TourEquipmentController extends AppBaseController
{
    /** @var  TourEquipmentRepository */
    private $tourEquipmentRepository;
    private static $slug = 'tour-equipment';

    public function __construct(TourEquipmentRepository $tourEquipmentRepo)
    {
        $this->tourEquipmentRepository = $tourEquipmentRepo;
    }
...
//        $query=TourEquipment::select('*');
//        $dataTypeContent = self::getData(self::$slug, $request, $query);
//        $browserable_rows = self::getFields(self::$slug, $this);
//
//        return view('tour_equipments.index')
//            ->with(['tourEquipments'=>$dataTypeContent,'rows'=>$browserable_rows]);
    }
