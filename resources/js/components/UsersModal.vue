<template>
    <div>
        <button id="show-modal" class="btn btn-basic" @click="showModal = true"><i class="mdi mdi-plus-circle"></i> {{translate('Add users')}}</button>
        <div class="modal-mask" v-if="showModal==true">
            <div class="modal-wrapper"  @click="showModal=false" >
                <div class="modal-container" @click.stop="">

                    <div class="modal-header">
                        <slot name="header">
                            <span class="navigation-back" @click="showModal=false">
                                <i class="mdi mdi-arrow-left"></i> <span class="navigation-back--text">{{translate('Back')}}</span>
                            </span>
                        </slot>
                    </div>

                    <div class="modal-body">
                        <slot name="body">
                            <span class="ml-3"> {{translate('Users')}} </span>
                            <input type="text" v-model="search" class="form-control" @input="onCategorieChange($event)">
                            </input>
                            <div class="tabl ml-3">
                                <div v-if="equipment.length>0">
                                    <div class="equipment-list" v-for="item in equipment" :key="item.id">
                                        <span v-if="selectedEquip.includes(item.id)">
                                            <input type="checkbox" @change="onEquipmentUnCheck($event)" :value="item.id" checked="checked"> {{item.name}}
                                        </span>
                                        <span v-else>
                                            <input type="checkbox" @change="onEquipmentCheck($event)" :value="item.id"> {{item.name}}
                                        </span>
                                    </div>
                                </div>
                                <div v-else>
                                    {{translate('Not found')}}
                                </div>
                            </div>
                        </slot>
                    </div>
                    <div class="modal-footer">
                        <slot name="footer">
                            <form action="/equipment/add" method="get" class="modal-footer--form" v-if="!hasCallback">
                                <span v-if="selectedEquip.length>0" class="modal-footer--counter">
                                    {{selectedEquip.length}} {{translate('selected')}}
                                    <input type="hidden" name="equipment_ids[]" v-for="item in selectedEquip" :key="item.id" :value="item">
                                </span>
                                <button class="btn btn-basic add-equipment"><i class="mdi mdi-plus-circle"></i> {{translate('Add')}}</button>
                            </form>
                            <span class="modal-footer--form" v-else>
                               <span v-if="selectedEquip.length>0" class="modal-footer--counter">
                                    {{selectedEquip.length}} {{translate('selected')}}
                                    <input type="hidden" name="equipment_ids[]" v-for="item in selectedEquip" :key="item.id" :value="item">
                                </span>
                                <button class="btn btn-basic add-equipment" @click="addEquip"><i class="mdi mdi-plus-circle"></i> {{translate('Add')}}</button>
                            </span>
                        </slot>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    export default {
        name: "SimpleModal",
        props:["categories","translations","localeLang","hasCallback","callback","selected"],
        data() {
            return {
                showModal: false,
                equipment: false,
                search:'',
                selectedEquip:[]
            }
        },
        methods:{
            //Генерирует перевод строк на основе json массива из props для компонента
            /*
            * Пример данных на входе. Переводы берутся из Laravel
            * :translations="[
                        {'Back':'{{__('Back')}}'},
                        {'Add':'{{__('Add')}}'},
                        {'Category':'{{__('Category')}}'},
                        {'Add equipment':'{{__('Add equipment')}}'}
              ]"
            * */
            translate(word){
                if(this.translations!=='undefined'){
                    for (const [k, v] of Object.entries(this.translations)) {
                        for (const [key, value] of Object.entries(v)){
                            if(key===word){
                                return value;
                            }
                        }

                    }
                }
                return word;
            },
            loadEquipment(ctx){
                axios.get('/user/find/?search='+this.search).then(function (response) {
                    // handle success
                    ctx.equipment=response.data;
                });
            },
            onCategorieChange(event) {
                this.loadEquipment(this,event.target.value);
            },
            onEquipmentCheck(event){
                console.log(event.target);
                this.selectedEquip.push(parseInt(event.target.value));
            },
            onEquipmentUnCheck(event){
                console.log(event.target);
                this.selectedEquip=this.selectedEquip.filter(function(e) { return e != event.target.value });
            },
            addEquip(){
                this.$emit('callback', this.selectedEquip);
                this.showModal=false;
            }

        },
        created() {
            //this.loadEquipment(this);
            if(this.selected)
                this.selectedEquip=JSON.parse(JSON.stringify(this.selected)).split(',').map(Number);
        },

    }
</script>

<style scoped>
    .modal-mask {
        position: fixed;
        z-index: 9998;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;
        /*background-color: rgba(0, 0, 0, 0.5);*/
        display: table;
        transition: opacity 0.3s ease;
    }
    .add-equipment{
        margin-bottom:23px;
    }
    .modal-wrapper {
        display: table-cell;
        vertical-align: bottom;
    }

    .equipment-list{
        min-height:40px;
    }
    .tabl{
        margin-top: 20px;
    }
    .modal-container {
        position: absolute;
        width: 386px;
        height: 540px;
        top: 0px;
        right: 20px;
        bottom: 20px;
        background: #ffffff;
        box-shadow: 0px 3.5px 50px rgba(0, 0, 0, 0.1);
        margin: 125px auto;
        border-radius: 15px;
        transition: all 0.3s ease;
    }
    .modal-footer--counter{
        display: flex;
        padding: 10px;
    }
    .modal-footer--form{
        display: flex;
    }


</style>
